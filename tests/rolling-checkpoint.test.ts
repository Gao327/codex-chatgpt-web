...
test("Luna checkpoint replaces only exact-parent history and preserves the current native turn", () => {
  ...
  const stale = store.apply(repeatedAnswerWithoutCheckpoint);
...
test("Luna checkpoints remain process-local across default, no-store, and explicit-store continuations", () => {
  const store = new ChatGptLunaCheckpointStore();
  const threadId = "thread_luna_privacy";
  const history: Record<string, unknown>[] = [];
  for (const [index, storage] of [undefined, false, true].entries()) {
    const turnId = `turn_privacy_${index}`;
    history.push(message("user", `Continue private task ${index}`, turnId));
    const source = request(threadId, turnId, history);
    if (storage !== undefined) (source._rawBody as Record<string, unknown>).store = storage;
    if (index > 0) expect(store.apply(source).applied).toBeTrue();
    const answer = `Private answer ${index}`;
    store.commit(source, {
      checkpoint: { version: 2, summary: "Private context carried from earlier turns." },
      answerHash: hashChatGptLunaAnswer(answer),
    }, answer);
    history.push(message("assistant", answer, turnId));
    const next = request(threadId, `turn_after_${index}`, [
      ...history,
      message("user", "Continue", `turn_after_${index}`),
    ]);
    expect(store.apply(next).applied).toBeTrue();
    const restarted = new ChatGptLunaCheckpointStore().apply(next);
    expect(restarted.applied).toBeFalse();
    expect(restarted.parsed).toBe(next);
    expect(restarted.reason).toContain("no checkpoint");
  }
});

test("expired Luna checkpoints fall back to canonical history", () => {
  let now = 1_000;
  const store = new ChatGptLunaCheckpointStore(() => now);
  const source = request("thread_expiry", "turn_source", [message("user", "Start", "turn_source")]);
  const answer = "Completed.";
  store.commit(source, { checkpoint, answerHash: hashChatGptLunaAnswer(answer) }, answer);
  const next = request("thread_expiry", "turn_next", [
    message("assistant", answer, "turn_source"),
    message("user", "Continue", "turn_next"),
  ]);
  expect(store.apply(next).applied).toBeTrue();
  now += 30 * 24 * 60 * 60_000 + 1;
  expect(store.apply(next)).toEqual({ parsed: next, applied: false, reason: "no checkpoint for the exact parent answer" });
});
