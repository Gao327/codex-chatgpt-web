...
const en = {
  ...
  mcpStepTwoHint: "Finish or stop running Codex tasks before connecting, including tasks using native Codex models. This step restarts the shared runtime. Once it succeeds and the tunnel is running, add the MCP connector in ChatGPT.",
...
const zh: Record<keyof typeof en, string> = {
  ...
  mcpStepTwoHint: "连接前，请先完成或停止正在运行的 Codex 任务，包括使用原生 Codex 模型的任务。此步骤会重启共享运行时。成功且 Tunnel 正在运行后，即可在 ChatGPT 中添加 MCP 连接器。",
...
const ja: Record<keyof typeof en, string> = {
  ...
  mcpStepTwoHint: "接続前に、ネイティブ Codex モデルを含む実行中の Codex タスクを完了または停止してください。この手順では共有ランタイムが再起動します。成功してトンネルが起動したら、ChatGPT に MCP コネクタを追加できます。",
